...
function socketHandler(io) {
    io.on('connection', (socket) => {
        ...
        socket.on('request-chat', async ({ customerId }) => {
            try {
                ...
                if (existingRoom) {
                    ...
                    if (staffSocketData) {
                        for (const staffSocketId of staffSocketData.socketIds) {
                            io.to(staffSocketId).emit('incomingChat', {
                                roomId: existingRoom._id.toString(),
                                customerId,
                                message: 'Bạn có một cuộc trò chuyện mới (phòng đã tồn tại)',
                            });
                        }
                    }
                ...
                if (staffSocketData) {
                    for (const staffSocketId of staffSocketData.socketIds) {
                        io.to(staffSocketId).emit('incomingChat', {
                            ...
                            customerId,
                            message: 'Bạn có một cuộc trò chuyện mới.',
                        });
                    }
                }
        ...
        // --- accept-chat (nhân viên tham gia phòng) ---
        socket.on('accept-chat', async ({ roomId }) => {
            socket.join(roomId);
            console.log(`👨‍💼 Staff ${socket.userId} accepted and joined room ${roomId}`);

            // Gửi về thông tin phòng để client load giao diện ngay
